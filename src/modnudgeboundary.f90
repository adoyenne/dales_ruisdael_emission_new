...
contains
    ...
    subroutine initnudgeboundary
        ...
        use modglobal,   only : ifnamopt, fname_options, imax, jmax, dx, dy, i1, j1, k1, &
        						ih, jh, lwarmstart, kmax, zf, checknamelisterror, nsv, &
        						rtimee
    ...
    subroutine read_initial_fields
        ...
        if (lnudge_boundary_sv) then
			...
			!sv0(2:i1,2:j1,1:kmax,svskip+1:nsv)=613. !400ppm
		
            open(777, file=input_file_sv, form='unformatted', status='unknown', action='read', access='stream')
            do isv = svskip+1,nsv
                read(777) sv0(2:i1,2:j1,1:kmax,isv) !if CAMS input is used for BCs, it is in ug/g (==1e6*kg/kg) Arseni
            end do
            close(777)
    ...
    subroutine read_new_LBCs_sv(time)
        ...
        if (myidx == 0 .or. myidx == nprocx-1 .or. myidy == 0 .or. myidy == nprocy-1) then
            ...
            !lbc_sv(2:i1,2:j1,1:kmax,svskip+1:nsv ,2)=613. !400ppm
