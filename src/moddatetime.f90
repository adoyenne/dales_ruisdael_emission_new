...
module moddatetime
  ...
  subroutine initdatetime
    ...
    call mpi_bcast(l_datetime, 1, mpi_logical, 0, comm3d, ierr)
    call mpi_bcast(startyear,  1, mpi_integer, 0, comm3d, ierr)
    call mpi_bcast(startmonth, 1, mpi_integer, 0, comm3d, ierr)
    call mpi_bcast(startday,   1, mpi_integer, 0, comm3d, ierr)
    call mpi_bcast(starthour,  1, mpi_integer, 0, comm3d, ierr)
    call mpi_bcast(timezone,   1, mpi_integer, 0, comm3d, ierr)
    ...
    if (l_datetime) then
      ...
      datex(4) = mod(int(starthour) + timezone + 24, 24) ! hour
      datex(5) = 0                ! minutes
      datex(6) = 0                ! seconds
  ...
  subroutine datetime
    ...
    use modglobal, only : rtimee, xtime, lwarmstart 
    ...
    !if(lwarmstart) then !Arseni do not need to add int(xtime) if lwarmstart, because it is already included in rtimee:  
    !	xhr = mod(itimee/3600 + timezone + 24, 24)
    !else
    !	xhr = mod(itimee/3600 + int(xtime) + timezone + 24, 24)
    !endif
