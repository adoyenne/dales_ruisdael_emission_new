...
!> Calculates surface resistance, temperature and moisture using the Land Surface Model
  subroutine do_lsm
    ...
    real     :: rsAgs, rsCO2, fw, Resp, wco2, conv_to_ppb, conv_from_ppb !Variables for AGS
	...
	conv_to_ppb   = MW_Air/MW_CO2/rhof(1)*1000.
    conv_from_ppb = 1./conv_to_ppb
        ...
        if (lrsAgs) then
          if (.not. linags) then !initialize AGS
            ...
            if(lCHon) then !Chemistry is on
              ...
              indCO2 = 4 !it is 1 in original code but here I put scalar of svco2sum Arseni
            endif !Is chemistry or bulk_micro on?
            linags = .true.
          endif !linags
		  
		  !indCO2 traser should be the sum of all CO2 probably !Arseni
		  

          CO2ags  = svm(i,j,1,indCO2)*(MW_Air/MW_CO2)*1000. !From ug/g (usual DALES standard) to ppb ! Arseni
		  ...
		  !CO2 absolute concentration [mol x m^-3]:
		  
          !co2abs   = CO2ags * (MW_CO2/MW_Air) * rhof(1)
		  
		  co2abs   = CO2ags * conv_from_ppb  !Arseni
...
!          Resp     = R10 * (1 - fw)*(1+ustar(i,j)) * exp( Eact0 / (283.15 * 8.314) * (1.0 - 283.15 / ( thl0(i,j,1) )))
          ...
          !Sum of photosynthesis (in) and respiration (out) CO2 fluxes:
          !wco2     = (An + Resp) * (MW_Air/MW_CO2) * (1.0/rhof(1)) ! In ppm m/s
		  wco2     = (An + Resp) * conv_to_ppb !
		  

		  !CO2flux(i,j) = wco2 * 1000.0

          CO2flux(i,j) = (wco2*(MW_CO2/MW_Air))/1000.		  
