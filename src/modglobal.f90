...
subroutine calculate_year_month_day(xyear, xday, rtimee,year, month, day)
    implicit none
    ...
    day = xday + int(rtimee/(3600*24))		!to account for warmstart simulations
    
    if((int(rtimee/(3600*24)).ge.1).and.(day.gt.365)) then !to account for warmstart simulations 
    	
    	
    	if ((mod(year, 4) == 0) .and. (mod(year, 100) /= 0 .or. mod(year, 400) == 0).and.(day.gt.366)) then
    ...
    ! Assign the days_in_month array for a non-leap year
...
end subroutine calculate_year_month_day
