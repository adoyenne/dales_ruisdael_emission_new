    ...
    if(myid==0) then
      ...
      if (lnetcdf) then
        ...
        if(isurf==1) then
          ...
          if (lags) then
            call ncinfo(ncname(vi,:),'an_co2','Net CO2 assimilation','ug/g m s-1','time') !not ppb, see modlsm
            vi = vi+1
            call ncinfo(ncname(vi,:),'resp_co2','CO2 respiration soil','ug/g m s-1','time') !not ppb, see modlsm
            vi = vi+1
          end if
        end if
