...
!NetCDF variables
  ...
  integer :: ncid3(100)
  integer :: nrec1(100) = 0
  integer :: nrec2(128) = 0  !Should be 100 in original Arseni
  integer :: nrec3(100) = 0
...
!> Initializing Crosssection. Read out the namelist, initializing the variables
  subroutine initcrosssection
    use mpi
    use modmpi,   only :myid,mpierr,my_real,mpierr,comm3d,mpi_logical,mpi_integer,cmyid,cmyidx,cmyidy,myidx,myidy
   ...
   implicit none
    ...
    call MPI_BCAST(crossplane (1:100), 100, MPI_INTEGER, 0, comm3d, mpierr)
    call MPI_BCAST(crossortho (1:100), 100, MPI_INTEGER, 0, comm3d, mpierr)
    ...
    if (lnetcdf) then
      ...
      do cross=1,nxz
         if (crossplane(cross) >= 2 .and. crossplane(cross) <= j1) then
            ...
            vars(:,:,10)  = cloudnr(2:i1, crossplane(cross), 1:kmax)
            vars(:,:,11:nvar) = svm(2:i1, crossplane(cross), 1:kmax, :)
...
!> Do the xy crosssections and dump them to file
  subroutine wrthorz
    use modglobal, only : imax,jmax,i1,j1,rlv,cp,rv,rd,cu,cv,cexpnr,ifoutput,rtimee
    use modfields, only : um,vm,wm,thlm,qtm,svm,thl0,qt0,ql0,e120,exnf,thvf,cloudnr
    ...
    if (lnetcdf) then
       ...
       do cross=1,nyz
          if (crossortho(cross) >= 2 .and. crossortho(cross) <= i1) then
             ...
             vars(:,:,10) = cloudnr(crossortho(cross), 2:j1, 1:kmax)
             vars(:,:,11:nvar)= svm(crossortho(cross), 2:j1, 1:kmax, :)
